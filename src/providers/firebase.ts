...
export class FirebaseProvider {
  ...
  sendFriendRequest(userId) {
    ...
    this.dataProvider.getRequests(loggedInUserId).snapshotChanges().take(1).subscribe((requests) => {
      ...
      this.tempData = requests;
      if (requests.payload.val() != null && this.tempData.payload.val().requestsSent != null)
        requestsSent = this.tempData.payload.val().requestsSent;
      ...
      this.angularfire.object('/requests/' + loggedInUserId).update({
        ...
        this.dataProvider.getRequests(userId).snapshotChanges().take(1).subscribe((requests) => {
          ...
          if (requests.payload.val() != null && this.tempData1.payload.val().friendRequests != null)
            friendRequests = this.tempData1.payload.val().friendRequests;
  ...
  deleteFriendRequest(userId) {
    ...
    this.dataProvider.getRequests(loggedInUserId).snapshotChanges().take(1).subscribe((requests) => {
      this.tempData = requests;
      friendRequests = this.tempData.payload.val().friendRequests;
      console.log(friendRequests);
      ...
      this.angularfire.object('/requests/' + loggedInUserId).update({
        ...
        this.dataProvider.getRequests(userId).snapshotChanges().take(1).subscribe((requests) => {
          this.tempData1 = requests;
          requestsSent = this.tempData1.payload.val().requestsSent;
  ...
  acceptFriendRequest(userId) {
    ...
    this.dataProvider.getUser(loggedInUserId).snapshotChanges().take(1).subscribe((account) => {
      ...
      var friends = this.tempData.payload.val().friends;
      if (!friends) {
        friends = [userId];
      } else {
        friends.push(userId);
      ...
      this.dataProvider.getUser(loggedInUserId).update({
        ...
        this.dataProvider.getUser(userId).snapshotChanges().take(1).subscribe((account) => {
          this.tempData1 = account;
          var friends = this.tempData1.payload.val().friends;
          if (!friends) {
