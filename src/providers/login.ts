  ...
  facebookLogin() {
    ...
    if (this.platform.is('core')) {
      ...
      this.afAuth.auth.signInWithPopup(new firebase.auth.FacebookAuthProvider()).then(fbRes => {
        ...
        this.tempData1 = fbRes;

        let fbcredential = firebase.auth.FacebookAuthProvider.credential(this.tempData1.credential.accessToken);
        console.log(fbcredential);
        this.loadingProvider.show();
        firebase.auth().signInWithCredential(fbcredential).then((success) => {

          this.tempData = fbRes.additionalUserInfo.profile;

          // let data = fbRes.additionalUserInfo.profile;
          let uid = firebase.auth().currentUser.uid;
          console.log(uid);
          if (fbRes.additionalUserInfo.isNewUser == true)
            this.createNewUser(uid, this.tempData.first_name, uid, this.tempData.email, 'I am available', 'Facebook', this.tempData.picture.data.url, this.tempData.gender);
  ...
  googleLogin() {
    if (this.platform.is('core')) {
      this.afAuth.auth.signInWithPopup(new firebase.auth.GoogleAuthProvider()).then(gpRes => {
        console.log(gpRes);
        this.tempData = gpRes;
        let credential = firebase.auth.GoogleAuthProvider.credential(this.tempData.credential.idToken, this.tempData.credential.accessToken);
        firebase.auth().signInWithCredential(credential)
          .then((success) => {
            console.log(success);
            let uid = firebase.auth().currentUser.uid;
            ...
            if (gpRes.additionalUserInfo.isNewUser == true)
              this.createNewUser(uid, this.tempData.name, uid, this.tempData.email, 'I am available', 'Google', this.tempData.picture, this.tempData.gender);
            this.loadingProvider.hide();
          })
          .catch((error) => {
            this.loadingProvider.hide();
      ...
      this.googleplus.login({
        ...
        firebase.auth().signInWithCredential(credential)
          .then((data) => {
            ...
            this.loadingProvider.hide();
            let uid = firebase.auth().currentUser.uid;
            this.createNewUser(uid, data.displayName, uid, data.email, 'I am available', 'Google', this.tempData.imageUrl);
          })
          .catch((error) => {
            this.loadingProvider.hide();
            this.alertProvider.showErrorMessage(error["code"]);
