            ...
            this.dataProvider.getUser(conversation.key).snapshotChanges().subscribe((user) => {
              ...
              this.dataProvider.getConversation(conversation.conversationId).snapshotChanges().subscribe((obj) => {
                ...
                if (obj.payload.val() != null) {
                  let lastMessage = this.tempData1.payload.val().messages[this.tempData1.payload.val().messages.length - 1];
                  conversation.date = lastMessage.date;
                  conversation.sender = lastMessage.sender;
                  // Set unreadMessagesCount
                  conversation.unreadMessagesCount = this.tempData1.payload.val().messages.length - conversation.messagesRead;
                  console.log(this.tempData1.payload.val().messages.length + "-" + conversation.messagesRead);
