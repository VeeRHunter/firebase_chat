      ...
      if (group.payload.exists()) {
        ...
        this.dataProvider.getGroupMessages(group.key).snapshotChanges().subscribe((messagesRes) => {
          ...
          if (this.messages != null && this.messages != undefined) {
            ...
            if (this.tempData2.length > this.messages.length) {
              let message = messages[this.tempData2.length - 1];
              this.dataProvider.getUser(message.sender).snapshotChanges().subscribe((user) => {
                this.tempData3 = user;
                message.avatar = this.tempData3.payload.val().img;
              });
            ...
            this.messages = [];
            this.tempData2.forEach((message) => {
              console.log(message)
              this.dataProvider.getUser(message.sender).snapshotChanges().subscribe((user) => {
                this.tempData4 = user;
                if (user.key != null) {
