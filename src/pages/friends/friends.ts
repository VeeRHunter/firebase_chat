...
export class FriendsPage {
  ...
  private tempData2: any = {};
  private tempData3: any = {};
  ...
  ionViewDidLoad() {
    ...
    this.dataProvider.getRequests(firebase.auth().currentUser.uid).snapshotChanges().subscribe((requestsRes) => {
      ...
      this.tempData1 = requests;
      console.log(requests);
      if (requests != null) {
        if (this.tempData1.friendRequests != null && this.tempData1.friendRequests != undefined)
          this.friendRequestCount = this.tempData1.friendRequests.length;
  ...
  getFriends() {
    ...
    this.dataProvider.getCurrentUser().snapshotChanges().subscribe((account) => {
      ...
      this.tempData2 = account;

      if (this.tempData2.payload.val() != null && this.tempData2.payload.val().friends != null) {
        for (var i = 0; i < this.tempData2.payload.val().friends.length; i++) {
          this.dataProvider.getUser(this.tempData2.payload.val().friends[i]).snapshotChanges().subscribe((friend) => {
            if (friend.key != null) {
      ...
      this.dataProvider.getRequests(this.account.userId).snapshotChanges().subscribe((requestsRes) => {
        ...
        if (requests != null) {
          if (this.tempData1.friendRequests != null && this.tempData1.friendRequests != undefined) {
            this.friendRequests = [];
            this.friendRequestCount = this.tempData1.friendRequests.length;
            this.tempData1.friendRequests.forEach((userId) => {
          ...
          // requestsSent.
          if (this.tempData1.requestsSent != null && this.tempData1.requestsSent != undefined) {
            this.requestsSent = [];
            this.tempData1.requestsSent.forEach((userId) => {
  ...
  findNewFriends() {
    ...
    this.dataProvider.getUsers().snapshotChanges().subscribe((accounts) => {
      ...
      let acc = accounts.filter(c => {
        ...
        if (this.tempData1.payload.val().name == '' || this.tempData1.payload.val().name == ' ' || this.tempData1.payload.val().name == undefined) return false;
        if (this.tempData1.payload.val().publicVisibility == false) return false;
        return true;
      });
      ...
      this.dataProvider.getCurrentUser().snapshotChanges().subscribe((account) => {
        ...
        if (this.tempData2.payload.val() != null) {
          // console.log(account.payload.val().friends);
          if (this.tempData2.payload.val().friends != null) {
            this.tempData2.payload.val().friends.forEach(friend => {
        ...
        this.dataProvider.getRequests(account.key).snapshotChanges().subscribe((requests) => {
          this.tempData3 = requests;
          if (requests.payload.val() != null) {
            this.requestsSent = this.tempData3.payload.val().requestsSent;
            this.friendRequests = this.tempData3.payload.val().friendRequests;
          }
        });
      });
