...
export class MessagePage {
  ...
  public tempData4: any = {};
  public tempData5: any = {};
  public tempData6: any = {};
  ...
  ionViewDidLoad() {
    ...
    this.dataProvider.getUser(this.userId).snapshotChanges().subscribe((user) => {
      this.tempData1 = user;
      this.title = this.tempData1.payload.val().name;
    });
    ...
    this.angularfire.object('/accounts/' + this.loggedInUserId + '/conversations/' + this.userId).snapshotChanges().subscribe((conversation) => {
      this.tempData2 = conversation;
      if (conversation.payload.exists()) {
        // User already have conversation with this friend, get conversation
        this.conversationId = this.tempData2.payload.val().conversationId;
        ...
        this.dataProvider.getConversationMessages(this.conversationId).snapshotChanges().subscribe((messagesRes) => {
          ...
          this.tempData3 = messages;
          console.log(messages);
          if (messages == null)
            messages = [];
          if (this.messages) {
            // Just append newly added messages to the bottom of the view.
            if (this.tempData3.length > this.messages.length) {
              let message = messages[this.tempData3.length - 1];

              this.dataProvider.getUser(message.sender).snapshotChanges().subscribe((user) => {
                this.tempData4 = user;
                message.avatar = this.tempData4.payload.val().img;
              });
            ...
            this.messages = [];
            this.tempData3.forEach((message) => {
              this.dataProvider.getUser(message.sender).snapshotChanges().subscribe((user) => {
                this.tempData5 = user;
                message.avatar = this.tempData5.payload.val().img;
              });
              this.messages.push(message);
            });
