  ...
  ionViewDidLoad() {
    ...
    this.dataProvider.getUser(firebase.auth().currentUser.uid).snapshotChanges().subscribe((user) => {
      if (user.payload.val() != null) {
        this.tempData1 = user;
        this.friends = this.tempData1.payload.val().friends;
      }
    });
    ...
    this.dataProvider.getRequests(firebase.auth().currentUser.uid).snapshotChanges().subscribe((requests) => {
      ...
      if (requests.payload.val() != null) {
        this.tempData2 = requests;
        this.friendRequests = this.tempData2.payload.val().friendRequests;
        this.requestsSent = this.tempData2.payload.val().requestsSent;
      }
    });
  }
