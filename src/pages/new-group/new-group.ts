...
export class NewGroupPage {
  ...
  ionViewDidLoad() {
    ...
    this.dataProvider.getCurrentUser().snapshotChanges().subscribe((accountRes) => {
      ...
      if (this.tempData1.friends) {
        for (var i = 0; i < this.tempData1.friends.length; i++) {
          this.dataProvider.getUser(this.tempData1.friends[i]).snapshotChanges().subscribe((friendRes) => {
