...
export class AddMembersPage {
  ...
  private tempData4: any = {};
  private tempData5: any = {};
  private tempData6: any = {};
  ...
  ionViewDidLoad() {
    ...
    this.dataProvider.getGroup(this.groupId).snapshotChanges().subscribe((group) => {
      ...
      this.tempData1 = group;
      // Get group members
      if (this.tempData1.payload.val().members) {
        this.tempData1.payload.val().members.forEach((memberId) => {
        ...
        this.dataProvider.getCurrentUser().snapshotChanges().subscribe((account) => {
          ...
          if (this.tempData2.payload.val().friends) {
            for (var i = 0; i < this.tempData2.payload.val().friends.length; i++) {
              this.dataProvider.getUser(this.tempData2.payload.val().friends[i]).snapshotChanges().subscribe((friendRes) => {
